...
export default {
  ...
  methods: {
    ...
    get (explorer, type) {
      ...
      axios.post(link.url + 'aexplorer', explorer, { headers: { Authorization: 'Bearer ' + localStorage.getItem('token') } }).then(res => {
        ...
        if (res.data.ret === 'success') {
          ...
          this.openloading = false
          this.$toastr.error(res.data.motivo, 'Aviso:', util.toast)
        }
      }).catch(err => this.$toastr.error(err, 'Aviso:', util.toast))
    },
    getexplorer (crit) {
      if (this.criterio === '') {
